      ...
      <v-row align="center" justify="space-between" no-gutters>
        ...
        <v-col cols="auto" class="d-none d-lg-flex align-center gap-2">
          <v-btn
            ...
            :style="{ borderWidth: '2px', backdropFilter: 'blur(4px)' }"
          >
            Ordenes
          </v-btn>
          <v-btn
        ...
        <v-col cols="auto" class="d-flex d-lg-none align-center">
          <v-menu location="bottom">
            ...
            <v-list :style="{ background: isDark ? '#1a1a1a' : '#ffffff' }">
              ...
              <v-list-item @click="$emit('navigate', 'schedule')">
                <template v-slot:prepend>
                  <v-icon color="red">mdi-calendar-edit</v-icon>
                </template>
...
const reloadMechanic = (event: MouseEvent) => {
  event.preventDefault() // Evita que RouterLink navegue normal
  router.push('/mechanic').then(() => {
